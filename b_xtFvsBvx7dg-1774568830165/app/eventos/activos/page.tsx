...
export const metadata: Metadata = {
  title: 'Eventos Activos | BYHORMIGA',
  description: 'Eventos activos y disponibles para compra de BYHORMIGA en Montevideo.',
}

export default async function EventosActivosPage() {
  const allEvents = await getEvents()
  // Filter for active events (en-venta status)
  const events = allEvents.filter(event => event.status === 'en-venta')
      ...
      <main className="min-h-screen bg-[#0a0908] pt-32 pb-24">
        <div className="mx-auto max-w-[1600px] px-6 lg:px-12">
          ...
          <div className="mb-16 text-center">
            ...
            <h1 className="mt-4 text-5xl lg:text-7xl font-black font-heading tracking-[-0.035em] text-white">
              Eventos activos
            </h1>
          </div>
          ...
          {events.length > 0 ? (
            ...
            <div className="border border-white/[.08] bg-[#111111] px-6 py-12 text-center">
              <p className="text-sm text-gray-400">No hay eventos activos en este momento. Vuelve pronto para ver las próximas fechas.</p>
            </div>
          )}
        </div>
      </main>
